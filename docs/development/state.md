...
- **Cyrius port**: `src/error.cyr` + all jalwa-core + all jalwa-playback + all jalwa-ai + `src/ui/{renderers,app}.cyr` — green. `src/main.cyr` still the smoke stub. (`jlw_format_duration` now lives in `core/types.cyr`.)
...
| E — terminal UI | ⏳ in progress — `renderers` ✅, `app` ✅ (TUI state); widgets + tui (ratatui→ANSI CSI + darshana) next |
| F — desktop GUI | ☐ |
| G — binary + MCP | ☐ |

**Modules: all jalwa-core + all non-video jalwa-playback + all jalwa-ai + `ui/{renderers,app}` ✅. 15 / 33 rust modules ported.**
...
- core+playback suites: `error` 19 · `core_types` 72 · `playlist_io` 17 · `db_helpers` 34 · `db` 27 · `scanner` 31 · `watcher` 20 · `hardware` 55 · `dsp` 42 · `decode_thread` 15 · `engine` 46 · `mpris` 34 · `reco` 21 · `daimon` 42 · `fingerprint` 6 · `renderers` 28 · `app` 35 — all green (544). Bare `cyrius test` all green.
