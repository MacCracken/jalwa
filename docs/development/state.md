...
| F — desktop GUI | 🔨 in progress — portable logic done (`theme`, `art_cache`, `app`, `views`). **Draw layer (Phase 1, `gui/draw.cyr`) started**: a pure headless-testable **draw-command IR** (`JlwDrawCmd` + `JLW_DRAW_*`) + per-view builders over the ported helpers; `color_to_xrgb` bridge; **sidebar + transport** builders done (assert command kinds/geometry/colors/text). Path chosen: **CPU framebuffer** (puka `fb`), NOT mabda GPU; **dhancha is the design template, not a dep** (needs a newer toolchain + lacks slider/image/list/scroll widgets). Remaining Phase 1: equalizer/devices builders, `build_frame` (screen split + view dispatch), `key_to_action`, grid-mode library (sidebar/transport/library-scroll/now_playing/queue DONE). Phase 2: CPU raster. Phase 3: Wayland present shell (**unverifiable headless** — smoke-only). video also aethersafta-blocked. |
...
- 32 `.tcyr` suites, **1036 assertions, all green**. Includes `scanner_probe` 39 + `audio_decode` 34 (decode → f64→i16 → threaded async-playback → EQ + normalize) + `engine_playback` 27 (threaded play→TrackFinished→Stopped + transport pause/resume/live-volume/seek + open-cancels-playback, over the fnptr seam) + `mcp_serve` 34 + `tui_runloop` 38 + GUI (141 portable logic + `gui_draw` 43 (sidebar/transport/library-scroll/now_playing/queue builders)). Fixtures `tests/fixtures/probe.{wav,flac,mp3}`. Bare `cyrius test` (CI) all green.
