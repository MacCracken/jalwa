...
Backlogged: mid-track-live EQ + gapless (audio polish — decode/output/transport/EQ/normalize all done; these two are device-untestable/complex); OGG/AAC/MP4 decode (shravan decoders broken); GUI grid-mode library / clip-stack / interactive search text-entry / real album-art blit (minor polish; the draw→raster→control→Wayland-present stack is built + `jalwa gui` runs, smoke-only); `video_decode_thread`, `view_video`, video probe (tarang+aethersafta); MPRIS export (samvada is D-Bus client-only, hard-blocked).

## Tests

- 35 `.tcyr` suites, **1188 assertions, all green**. Includes `scanner_probe` 39 + `audio_decode` 34 (decode → f64→i16 → threaded async-playback → EQ + normalize) + `engine_playback` 27 (threaded play→TrackFinished→Stopped + transport pause/resume/live-volume/seek + open-cancels-playback, over the fnptr seam) + `mcp_serve` 34 + `tui_runloop` 38 + GUI (141 portable logic + `gui_draw` 74, `gui_raster` 42 (fill_rect/border/glyph/UTF-8 text + executor + full-frame render→PPM), `gui_input` 39 (evdev→key→action), `gui_control` 37 (apply_action per-view effects + evdev keystroke path)). Fixtures `tests/fixtures/probe.{wav,flac,mp3}`. Bare `cyrius test` (CI) all green.
