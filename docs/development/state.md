...
| C — playback + DSP | ✅ done — `dsp`, `decode_thread`, `engine`, `mpris`, **`audio` (real decode→output)**. `jalwa play` produces sound: shravan `flac/mp3/wav_decode` → volume → f64→i16 → **vani** ALSA/agnos out (blocking). First slice = synchronous CLI playback; threaded engine play + EQ-in-loop + gapless + OGG/AAC/MP4 decode deferred. D-Bus stubbed; video_decode_thread tarang-blocked. |
...
- 30 `.tcyr` suites, **941 assertions, all green**. Includes `scanner_probe` 31 + `audio_decode` 20 (shravan decode → f64→i16 conversion; fixtures `tests/fixtures/probe.{wav,flac,mp3}`) + `mcp_serve` 33 + `tui_runloop` 38 + GUI (141). Bare `cyrius test` (CI) all green. `main`: assembly builds & runs (2.33MB — folds shravan/sankoch/vani); `jalwa scan`, `jalwa play`, `jalwa mcp` end-to-end smoke-tested (`play` decodes real audio, falls back cleanly when no ALSA device); `poll(2)` verified on agnos.
