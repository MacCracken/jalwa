...
- **Cyrius port**: `src/error.cyr` + `src/core/{types,playlist_io,db,scanner,watcher,hardware}.cyr` (all jalwa-core) + `src/playback/{dsp,decode_thread,engine}.cyr` — green. `src/main.cyr` still the smoke stub.
...
| C — playback + DSP | ⏳ in progress — `dsp` ✅, `decode_thread` ✅, `engine` ✅ (facade state-mgmt; play/open/decode tarang-stubbed); `mpris` shape-only last; video_decode_thread backlogged |
| D — AI | ☐ |
| E — terminal UI | ☐ |
| F — desktop GUI | ☐ |
| G — binary + MCP | ☐ |

**Modules: all 6 jalwa-core + `playback/{dsp,decode_thread,engine}.cyr` ✅ (decode_loop + play/open decode tarang-stubbed). 9 / 33 rust modules ported.**
...
- `error` 19 · `core_types` 72 · `playlist_io` 17 · `db_helpers` 34 · `db` 27 · `scanner` 31 · `watcher` 20 · `hardware` 55 · `dsp` 42 · `decode_thread` 15 · `engine` 46 — all green (378). Bare `cyrius test` all green.
